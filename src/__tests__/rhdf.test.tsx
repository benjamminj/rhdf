...
describe('mutate', () => {
  describe('config object API', () => {
      ...
      const TestComponent = () => {
        ...
        return (
          <div>
            ...
            <p>{a.status === 'success' && a.data}</p>
            <button
              onClick={() => mutate(prev => (prev ? prev + 1 : undefined))}
            >
              update
            </button>
          </div>
    ...
    test('should allow updating without a cache value', async () => {
      const updater = jest.fn().mockResolvedValue('updated')
      const fetcher = jest.fn().mockResolvedValue('original')
  ...
  describe('cacheKey as string API', () => {
    test('should update the item in the cache', async () => {
      const updater = jest.fn().mockResolvedValue('updated')
      const fetcher = jest.fn().mockResolvedValue('original')
    ...
    test('should support an "onSuccess" callback', async () => {
      const updater = jest.fn().mockResolvedValue('updated')
      const fetcher = jest.fn().mockResolvedValue('original')
      const onSuccess = jest.fn()
      ...
      const TestComponent = () => {
        const a = useQuery<string>('/a', fetcher)
        const { mutate } = useMutation('/a', { onSuccess })
        return (
          <div>
            <p>{a.status === 'loading' && 'loading'}</p>
            <p>{a.status === 'success' && a.data}</p>
